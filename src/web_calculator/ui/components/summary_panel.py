...
class SummaryPanel(ctk.CTkFrame):
    def __init__(
        ...
        ctk.CTkLabel(self, text="Zlava (%)").grid(row=1, column=0, sticky="w", padx=8)
        discount_frame = ctk.CTkFrame(self, fg_color="transparent")
        discount_frame.grid(row=1, column=1, sticky="e")
        ...
        separator.grid(row=2, column=0, columnspan=2, sticky="we", pady=6, padx=8)

        ctk.CTkLabel(self, text="Spolu bez DPH", font=("Segoe UI", 12, "bold")).grid(row=3, column=0, sticky="w", padx=8)
        ctk.CTkLabel(self, textvariable=self._total_var, font=("Segoe UI", 12, "bold")).grid(row=3, column=1, sticky="e", padx=8)

        ctk.CTkLabel(self, text=f"DPH {int(self._vat_rate*100)} %").grid(row=4, column=0, sticky="w", padx=8)
        ctk.CTkLabel(self, textvariable=self._vat_var).grid(row=4, column=1, sticky="e", padx=8)

        ctk.CTkLabel(self, text="Spolu s DPH", font=("Segoe UI", 13, "bold")).grid(row=5, column=0, sticky="w", padx=8)
        ctk.CTkLabel(self, textvariable=self._total_vat_var, font=("Segoe UI", 13, "bold")).grid(row=5, column=1, sticky="e", padx=8)
